...
class VAEDecodeLACE:
    ...
    def INPUT_TYPES(s):
        return {"required": {
                             "vae": ("VAE", ), 
                             "latents_list": ("LIST", ),
                             "tile_size": ("INT", {"default": 512, "min": 32, "max": 4096, "step": 64})
                            }
                }
    RETURN_TYPES = ()
    FUNCTION = "decode"
    ...
    def decode(self, vae, latents_list, tile_size):
        ...
        for idx, latents in enumerate(latents_list):
            ...
            imgs = VAEDecode().decode(vae, latents)[0]
...
class TSC_KSampler:
    ...
    RETURN_TYPES = ("MODEL", "CONDITIONING", "CONDITIONING", "LATENT", "VAE", "IMAGE", "LIST",)
    RETURN_NAMES = ("MODEL", "CONDITIONING+", "CONDITIONING-", "LATENT", "VAE", "IMAGE", "LATENT_LIST",)
    OUTPUT_NODE = True
    FUNCTION = "sample"
    ...
    def sample(self, model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent_image,
        ...
        if not keys_exist_in_script("xyplot"):
            ...
            samples, images, gifs, preview, full_latent = process_latent_image(model, seed, steps, cfg, sampler_name, scheduler,
        ...
        # If XY Plot
            ...
            def process_values(model, refiner_model, add_noise, seed, steps, start_at_step, end_at_step,
                ...
                if capsule_result is None:
                    ...
                    samples, images, _, _, full_latent = process_latent_image(model, seed, steps, cfg, sampler_name, scheduler, positive, negative,
        ...
        if sampler_type == "sdxl":
            sdxl_tuple = original_model, original_clip, original_positive, original_negative,\
                original_refiner_model, original_refiner_clip, original_refiner_positive, original_refiner_negative
            result = (sdxl_tuple, latent_list, optional_vae, output_images, full_latent[0])
        else:
    ...
    def sample_adv(self, model, add_noise, noise_seed, steps, cfg, sampler_name, scheduler, positive, negative,
        ...
        return super().sample(model, noise_seed, steps, cfg, sampler_name, scheduler, positive, negative,
               ...
               optional_vae=optional_vae, script=script, add_noise=add_noise, start_at_step=start_at_step,end_at_step=end_at_step,
                       return_with_leftover_noise=return_with_leftover_noise,sampler_type="advanced", LACE_Range=LACE_Range, LACE_step=LACE_step)
...
# NODE MAPPING
NODE_CLASS_MAPPINGS = {
    "KSampler (LACE)": TSC_KSampler,
    "KSampler Adv. (LACE)":TSC_KSamplerAdvanced,
    "KSampler SDXL (LACE)": TSC_KSamplerSDXL,
